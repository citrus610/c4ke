        ...
        for (i32 color = WHITE; color < 2; color++) {
            ...
            for (i32 type = PAWN; type < TYPE_NONE; type++) {
                ...
                for (; mask;) {
                    ...
                    // PST
                    eval += MATERIAL[type] + (get_data(type * 8 + square / 8) + get_data(type * 8 + square % 8 + INDEX_PST_FILE)) * SCALE_PST + OFFSET_PST;
                    ...
                    if (!type) {
                        ...
                        if (pawns_phalanx & 1ull << square)
                            eval += get_data(square / 8 + INDEX_PHALANX) * SCALE_PHALANX + OFFSET_PHALANX;
